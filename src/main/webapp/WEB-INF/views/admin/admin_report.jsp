...
<div style="width: 30%; height:70px; border:1px solid black; float:left;" onclick="load('admin_report.all');">
	...
	<br>
	<form action = "admin_reportSearchModifycnt.all?${_csrf.parameterName}=${_csrf.token}" name ="admin_approvesellerSearchAuth" method="post" onsubmit="return checkcart3();">
	<input type = "hidden" name = "${_csrf.parameterName}" value = "${_csrf.token}">		
		<h2 align="center">불량셀러신고</h2>
		<br>
		<table align="center" style="border-collapse: collapse;">
			<tr>
				...
				<td width=50></td>
				<td>아이디 :</td>
				<td><input type="text" name="searchInput3" style="height: 13px;"></td>
				<td><input type="button" value="검색" onclick="return searchPro3();"></td>
				<td width=50></td>
				<td>누적신고횟수 수정 :</td>
				<td><input type="text" name="reportModifyCnt" id="reportModifyCnt" style="height: 13px;"></td>
		...
		<c:forEach var = "dto" items="${dtos}">
			<tr>
				<td style="text-align: center;">
					<input type="checkbox" name="check3" id="check3" value="${dto.sel_id}">
				</td>
				<td>${dto.sel_id}</td>
				<td>${dto.sel_name}</td>
				<td>${dto.sel_email}</td>
    		...
    		<th align="center">
    		 ...
    		 <c:if test="${cnt > 0 }">
    			...
    			<c:if test="${startPage>pageBlock}">
    				<a href="admin_report.all">[◀◀]</a>
    				<a href="admin_report.all?pageNum=${startPage - pageBlock }">[◀]</a>
    			</c:if>
    			<!-- 블록내의 페이지 번호 -->
    			<c:forEach var="i" begin="${startPage}" end="${endPage}">
    				...
    				<c:if test="${i != currentPage}">
    					<a href="admin_report.all?pageNum=${i}">[${i}]</a>
    				</c:if>
    			</c:forEach>
    			<!-- 다음블록 [▶] /마지막[▶▶]   특수문자:ㅁ 한자키 -->
    			<c:if test="${pageCount>endPage}">
    				<a href="admin_report.all?pageNum=${startPage + pageBlock }">[▶]</a>
    				<a href="admin_report.all?pageNum=${pageCount}">[▶▶]</a>
    			</c:if>
    	     </c:if> 
    		</th>
   		</tr>
